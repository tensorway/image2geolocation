...
task = Task.init(project_name="image2geolocation", task_name="mobilenet_ranking")
...
# %%
model = BenchmarkModel('mobilenetv2')
...
for ep in range(4*20):
    for ibatch, train_dict in enumerate(train_dataloader):
        ...
        imgs = []
        for f, b, l, r in zip(train_dict['images']):
            imgs += [f, b, l, r]
        imgs = th.cat(imgs, dim=0)

        embeddings = model.embed(imgs, device)
        label_mat = 
